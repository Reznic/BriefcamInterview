...
def test_ground_truth_samples_of_line2d():
    ...
    * Assert the returned point sample is on the line equation
    ...
    assert len(gt_samples) == num_of_samples, \
...
def test_ground_truth_samples_of_circle2d():
    """Test that ground truth generator can generate sample points on a 2d line.
    ...
    * Assert the returned point sample is on the line equation
    """
    circle = Circle2D()
    circle.center = np.array([1, 1])
    circle.radius = 10

    num_of_samples = 5

    ground_truth_generator = GroundTruthSamplesGenerator()
    gt_samples = circle.accept(ground_truth_generator,
                               num_of_samples=num_of_samples,
                               samples_range=100)
    ...
    for sample in gt_samples:
        ...
        distance_sample_to_center = np.linalg.norm(circle.center - sample)
        assert distance_sample_to_center == approx(circle.radius,
                                                   NUMERIC_PERCISION), \
            f"Ground truth generator returned a point " \
            f"{sample} not on the circle perimeter"
